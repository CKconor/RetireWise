          ...
          >
            <p className={`font-display text-xl ${
              milestone.reached
                ? 'text-teal-700'
                : milestone === nextMilestone
                ? isOnTarget ? 'text-teal-600' : 'text-amber-600'
                : 'text-slate-300'
            }`}>
              {milestone.label}
            </p>
            <p className={`mt-0.5 text-xs ${
              milestone.reached ? 'text-teal-600/70' : 'text-slate-400'
            }`}>
              {formatCurrencyCompact(milestone.amount)}
            </p>
            {milestone.reached ? (
              <span className="mt-1.5 inline-flex items-center gap-1 text-xs font-semibold text-teal-600">
      ...
      {nextMilestone ? (
        <div className={`rounded-xl p-4 ${
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-semibold ${isOnTarget ? 'text-teal-800' : 'text-amber-800'}`}>
                Next: {formatCurrency(nextMilestone.amount)}
              </p>
              <p className={`text-xs ${isOnTarget ? 'text-teal-600' : 'text-amber-600'}`}>
            ...
            {nextMilestone.projectedReachAge && (
              <div className={`rounded-lg px-3 py-1.5 ${
                isOnTarget ? 'bg-teal-100/80' : 'bg-amber-100/80'
              }`}>
                <p className={`text-xs font-medium ${isOnTarget ? 'text-teal-700' : 'text-amber-700'}`}>
        ...
        <div className="rounded-xl bg-gradient-to-r from-teal-50 to-emerald-50/50 p-4 ring-1 ring-teal-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-teal-800">All milestones reached!</p>
